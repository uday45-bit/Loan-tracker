...
- **Dashboard** — Principal Outstanding (today), Total Payable (incl.
  interest, to closing), active loan/policy counts, and a **Monthly Cash
  Flow Needed** section that splits loan EMIs and insurance premiums into
  monthly-equivalent figures plus a combined total — the number to actually
  set aside each month. Plus upcoming payments and an outstanding-by-lender
  chart.
...
- **Insurance** — add Term or Health policies with a fixed premium paid
  quarterly, half-yearly, or yearly. Each policy card shows its
  monthly-equivalent cost and a "Mark premium paid" button that rolls the
  next-due date forward. Bulk-upload via CSV supported, same as Loans.
- **Month Lookup** — pick any year/month and see exactly what's due —
  loan installments (principal, interest, total) *and* insurance premiums —
  across everything, with a combined total.
