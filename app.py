...
def add_loan(name, loan_type, lender, principal, rate, tenure, start_date, notes):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO loans (name, loan_type, lender, principal, rate, tenure, start_date, notes, created_at) "
        "VALUES (?,?,?,?,?,?,?,?,?)",
        (name, loan_type, lender, principal, rate, tenure, start_date.isoformat(), notes, datetime.now().isoformat()),
    )
...
def loan_summary(loan_row) -> dict:
    ...
    emi = calculate_emi(loan_row["principal"], loan_row["rate"], tenure)
...
def page_dashboard():
    ...
    total_emi = sum(calculate_emi(r["principal"], r["rate"], r["tenure"]) for _, r in loans.iterrows())
...
def page_loans():
    ...
    with st.expander("➕ Add New Loan"):
        with st.form("add_loan_form", clear_on_submit=True):
            ...
            c3, c4 = st.columns(2)
            principal = c3.number_input("Principal Amount (₹) *", min_value=0.0, step=1000.0, value=500000.0)
            ...
            if submitted:
                if not name.strip():
                    ...
                    add_loan(name.strip(), loan_type, lender.strip(), principal, rate, int(tenure), start, notes.strip())
