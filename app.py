...
INSURANCE_FREQ_LABELS = {3: "Quarterly", 6: "Half-Yearly", 12: "Yearly"}
INSURANCE_FREQ_OPTIONS = list(INSURANCE_FREQ_LABELS.keys())
INSURANCE_TYPES = ["Term", "Health"]
...
def add_insurance(provider, policy_type, sum_assured, premium, frequency, start_date,
                   next_due_date, policy_end_date, notes=""):
    conn = get_conn()
    conn.execute(
        "INSERT INTO insurance (provider, policy_type, sum_assured, premium, frequency, start_date, "
        "next_due_date, policy_end_date, notes, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
        (provider, policy_type, sum_assured, premium, int(frequency), start_date.isoformat(),
         next_due_date.isoformat(), policy_end_date.isoformat() if policy_end_date else None,
         notes, datetime.now().isoformat()),
    )
    conn.commit()
    conn.close()
...
def update_insurance_progress(insurance_id, new_next_due_date):
    conn = get_conn()
    conn.execute(
        "UPDATE insurance SET next_due_date=? WHERE id=?",
        (new_next_due_date.isoformat(), int(insurance_id)),
    )
    conn.commit()
    conn.close()
...
def delete_insurance(insurance_id):
    conn = get_conn()
    conn.execute("DELETE FROM insurance WHERE id=?", (int(insurance_id),))
    conn.commit()
    conn.close()
...
def get_insurance() -> pd.DataFrame:
    conn = get_conn()
    df = pd.read_sql_query("SELECT * FROM insurance ORDER BY start_date ASC", conn)
    conn.close()
    return df
...
def build_insurance_schedule(ins_row, until_date: date) -> list:
    """
    Generates recurring premium due dates from next_due_date forward to
    `until_date` (or the policy's end date, whichever is sooner).
    """
    freq_months = int(ins_row["frequency"])
    premium = float(ins_row["premium"])
    due = parse_date_str(ins_row["next_due_date"])
    end_cap = parse_date_str(ins_row["policy_end_date"]) if pd.notna(ins_row["policy_end_date"]) and ins_row["policy_end_date"] else None
    hard_stop = min(until_date, end_cap) if end_cap else until_date

    schedule = []
    guard = 0
    while due <= hard_stop and guard < 500:
        schedule.append({"due": due, "amount": round(premium, 2)})
        due = due + relativedelta(months=freq_months)
        guard += 1
    return schedule


def insurance_metrics(ins_row) -> dict:
    monthly_equivalent = float(ins_row["premium"]) / int(ins_row["frequency"])
    next_due = parse_date_str(ins_row["next_due_date"])
    return dict(monthly_equivalent=monthly_equivalent, next_due=next_due, premium=float(ins_row["premium"]))


def month_due(loans_df: pd.DataFrame, year: int, month: int, insurance_df: pd.DataFrame = None) -> pd.DataFrame:
    rows = []
    for _, loan in loans_df.iterrows():
        for s in build_schedule(loan):
            if s["due"].year == year and s["due"].month == month:
                rows.append({
                    "Type": "Loan", "Name": loan["lender"], "Account": loan["account_type"],
                    "Due Date": fmt_date(s["due"]), "Principal": s["principal"],
                    "Interest": s["interest"], "Total Due": s["total"],
                })
    if insurance_df is not None:
        month_end = date(year, month, 28) + relativedelta(days=4)  # safely past month end
        month_end = month_end.replace(day=1) - relativedelta(days=1)
        for _, ins in insurance_df.iterrows():
            for s in build_insurance_schedule(ins, month_end):
                if s["due"].year == year and s["due"].month == month:
                    rows.append({
                        "Type": "Insurance", "Name": ins["provider"], "Account": f"{ins['policy_type']} Insurance",
                        "Due Date": fmt_date(s["due"]), "Principal": 0.0,
                        "Interest": 0.0, "Total Due": s["amount"],
                    })
...
def page_dashboard():
    st.title("Your Financial Portfolio")
    st.caption("Real outstanding balances and insurance premiums, projected forward.")

    loans = get_loans()
    insurance = get_insurance()
    if loans.empty and insurance.empty:
        st.info("No loans or insurance policies yet — add some from the **Loans** or **Insurance** pages.")
        return

    total_outstanding = total_payable = total_loan_monthly = 0
    ...
    for _, loan in loans.iterrows():
        ...
        total_loan_monthly += m["monthly_equivalent"]
        dist_rows.append({"Lender": loan["lender"], "Outstanding": m["outstanding"]})
        if m["next_due"] is not None:
            upcoming_rows.append({
                "Type": "Loan", "Name": f"{loan['lender']} — {loan['account_type']}",
                "Due": m["next_due"]["due"], "Amount": m["next_due"]["total"],
            })

    total_insurance_monthly = 0
    for _, ins in insurance.iterrows():
        im = insurance_metrics(ins)
        total_insurance_monthly += im["monthly_equivalent"]
        upcoming_rows.append({
            "Type": "Insurance", "Name": f"{ins['provider']} — {ins['policy_type']}",
            "Due": im["next_due"], "Amount": im["premium"],
        })

    total_monthly_obligation = total_loan_monthly + total_insurance_monthly
    ...
    c4.metric("Insurance Policies", len(insurance))

    st.subheader("💡 Monthly Cash Flow Needed")
    d1, d2, d3 = st.columns(3)
    d1.metric("Loan EMIs (monthly-equiv.)", inr(total_loan_monthly))
    d2.metric("Insurance (monthly-equiv.)", inr(total_insurance_monthly))
    d3.metric("Total Monthly Obligation", inr(total_monthly_obligation),
               help="Non-monthly premiums/installments (quarterly, half-yearly, yearly, bullet) are "
                    "normalized to a monthly-equivalent so you can see what to set aside each month, "
                    "on average — actual cash needed will spike in the specific months they fall due "
                    "(see Month Lookup).")
...
def page_insurance():
    st.title("Insurance")
    st.caption("Term and health insurance premiums — fixed amounts paid quarterly, half-yearly, or yearly.")

    with st.expander("➕ Add Insurance Policy"):
        with st.form("add_insurance_form", clear_on_submit=True):
            c1, c2 = st.columns(2)
            provider = c1.text_input("Insurer *", placeholder="e.g. LIC, HDFC Life, Star Health")
            policy_type = c2.selectbox("Policy Type *", INSURANCE_TYPES)
            c3, c4 = st.columns(2)
            sum_assured = c3.number_input("Sum Assured / Cover (₹)", min_value=0.0, step=10000.0, value=0.0)
            premium = c4.number_input("Premium Amount (₹) *", min_value=0.0, step=100.0, value=5000.0)
            c5, c6 = st.columns(2)
            frequency = c5.selectbox("Payment Frequency *", INSURANCE_FREQ_OPTIONS,
                                      format_func=lambda x: INSURANCE_FREQ_LABELS[x])
            start_date = c6.date_input("Policy Start Date *", value=date.today())
            c7, c8 = st.columns(2)
            next_due_date = c7.date_input("Next Premium Due Date *", value=date.today())
            has_end = c8.checkbox("Policy has a fixed end date", value=False)
            policy_end_date = None
            if has_end:
                policy_end_date = st.date_input("Policy End Date", value=date.today() + relativedelta(years=10))
            notes = st.text_area("Notes", placeholder="Optional notes")
            submitted = st.form_submit_button("Add Insurance", use_container_width=True)
            if submitted:
                if not provider.strip():
                    st.error("Insurer name is required.")
                else:
                    add_insurance(provider.strip(), policy_type, sum_assured or None, premium, frequency,
                                   start_date, next_due_date, policy_end_date, notes.strip())
                    st.success(f"'{provider}' added.")
                    st.rerun()

    with st.expander("📤 Bulk Upload Insurance (CSV)"):
        st.caption(
            "Columns: **provider, policy_type, sum_assured, premium, frequency, start_date, next_due_date, "
            "policy_end_date, notes**. `policy_type`: Term or Health. `frequency`: 3=Quarterly, 6=Half-Yearly, "
            "12=Yearly. Dates accept DD.MM.YYYY / DD-MM-YYYY / YYYY-MM-DD. Leave `policy_end_date` blank for "
            "an ongoing/auto-renewing policy."
        )
        template_df = pd.DataFrame([{
            "provider": "LIC", "policy_type": "Term", "sum_assured": 5000000, "premium": 12000,
            "frequency": 12, "start_date": "01-04-2025", "next_due_date": "01-04-2027",
            "policy_end_date": "", "notes": "",
        }])
        st.download_button("Download CSV template", template_df.to_csv(index=False).encode(),
                            file_name="insurance_template.csv", mime="text/csv")

        uploaded = st.file_uploader("Choose CSV file", type=["csv"], key="insurance_bulk_upload")
        if uploaded is not None:
            try:
                df = pd.read_csv(uploaded)
                required = {"provider", "policy_type", "premium", "frequency", "start_date", "next_due_date"}
                missing = required - set(df.columns)
                if missing:
                    st.error(f"Missing required column(s): {', '.join(sorted(missing))}")
                else:
                    st.dataframe(df, use_container_width=True, hide_index=True)
                    if st.button("Import all rows as insurance policies", type="primary"):
                        added, errors = 0, []
                        for i, row in df.iterrows():
                            try:
                                sa = row.get("sum_assured", None)
                                sum_assured = float(sa) if pd.notna(sa) and str(sa).strip() != "" else None
                                end_raw = row.get("policy_end_date", None)
                                policy_end_date = parse_date_str(end_raw) if pd.notna(end_raw) and str(end_raw).strip() != "" else None
                                add_insurance(
                                    str(row["provider"]).strip(), str(row["policy_type"]).strip(), sum_assured,
                                    float(row["premium"]), int(row["frequency"]), parse_date_str(row["start_date"]),
                                    parse_date_str(row["next_due_date"]), policy_end_date,
                                ...
                                errors.append(f"Row {i + 1}: {e}")
                        if added:
                            st.success(f"Imported {added} polic(y/ies).")
                        if errors:
                            st.warning("Some rows failed:\n" + "\n".join(errors))
                        if added:
                            st.rerun()
    ...
    for _, ins in insurance.iterrows():
        ...
        with st.container(border=True):
            ...
            top[0].markdown(f"**{ins['provider']}**  \n:gray[{ins['policy_type']} Insurance]")
            top[1].metric("Premium", f"{inr(ins['premium'])} / {INSURANCE_FREQ_LABELS[int(ins['frequency'])].split('-')[0]}")
            top[2].metric("Monthly-Equivalent", inr(im["monthly_equivalent"]))
            if top[3].button("🗑️", key=f"del_ins_{ins['id']}", help="Delete policy"):
                delete_insurance(ins["id"])
                st.rerun()

            end_txt = f" · Ends {fmt_date(ins['policy_end_date'])}" if pd.notna(ins["policy_end_date"]) and ins["policy_end_date"] else " · Ongoing (auto-renewing)"
            sa_txt = f" · Cover {inr(ins['sum_assured'])}" if pd.notna(ins["sum_assured"]) and ins["sum_assured"] else ""
            st.caption(f"Started {fmt_date(ins['start_date'])}{sa_txt}{end_txt} · Next due {fmt_date(im['next_due'])}")

            if st.button(f"✅ Mark {fmt_date(im['next_due'])} premium paid ({inr(ins['premium'])})", key=f"pay_ins_{ins['id']}"):
                new_due = im["next_due"] + relativedelta(months=int(ins["frequency"]))
                update_insurance_progress(ins["id"], new_due)
                st.rerun()


def page_month_lookup():
    st.title("Month Lookup")
    st.caption("See exactly what's due — loans and insurance — in a given month.")

    loans = get_loans()
    insurance = get_insurance()
    if loans.empty and insurance.empty:
        st.info("No loans or insurance policies yet.")
        return
    ...
    df = month_due(loans, int(year), int(month), insurance_df=insurance)
    if df.empty:
        st.write("Nothing due this month.")
        return
    ...
    loan_total = df[df["Type"] == "Loan"]["Total Due"].sum()
    insurance_total = df[df["Type"] == "Insurance"]["Total Due"].sum()

    c1, c2, c3 = st.columns(3)
    c1.metric("Loan EMIs Due", inr(loan_total))
    c2.metric("Insurance Due", inr(insurance_total))
    c3.metric("Total Cash Needed This Month", inr(df["Total Due"].sum()))
...
def main():
    ...
    page = st.sidebar.radio(
        "Navigate",
        ["📊 Dashboard", "📁 Loans", "🛡️ Insurance", "📅 Month Lookup", "📈 Interest Calculator"],
